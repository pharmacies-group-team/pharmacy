              ...
              <td style="display: flex; justify-content: start">
                <div class="user-table" d='{{ $order->image }}'>
                  ...
                  <img src="{{ asset(PharmacyEnum::PHARMACY_LOGO_PATH . $order->pharmacy->pharmacy->logo) }}"
                    alt="profile avatar" width="30" style="max-width: 30px; width: 100%" height="30">

                  <a href="{{ route('show.pharmacy.profile', $order->pharmacy->pharmacy->id) }}"
                    style="color: #3869BA">
                  ...
                  @if ($order->status === OrderEnum::NEW_ORDER)
                    <div>
                      <a href="{{ route('client.orders.cancel', $order->id) }}"
                        class="badge">@lang('action.cancel-order')</a>
                    </div>
                  @elseif($order->status === OrderEnum::UNPAID_ORDER)
                    <div>
                      <a href="{{ route('client.quotation.details', $order->quotation->id) }}"
