            ...
            @foreach ($users as $user)
              <tr>
                ...
                <td style="text-align: right "><a
                    href="{{ route('admin.users.profile', $user->id) }}">{{ $user->name }}</a> </td>
                ...
                {{-- action --}}
                <td style="text-align: right ">
                  @if (!$user->hasRole(\App\Enum\RoleEnum::SUPER_ADMIN))
                    <form>
                      <button wire:click.prevent="toggle({{ $user->id }})"
                        class="btn {{ $user->is_active ? 'btn-danger' : 'btn-primary' }} m-1">
    ...
    <x-modal title="اضافة اعلان" open="addModal" wire:ignore.self>
      <form enctype="multipart/form-data">
        ...
        <div class="form-group">
          ...
          <input wire:model="confirm_password" type="text"
            class="form-control @error('confirm_password') is-invalid @enderror" placeholder="تأكيد كلمة السر" />
        </div>
