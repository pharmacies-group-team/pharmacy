            ...
            <form method="POST" action="{{ route('login.custom') }}" class="row g-3 needs-validation" novalidate>
              ...
              <div class="col-12 input-group position-relative flex-nowrap">
                ...
                <input name="email" value="{{ old('email') }}" type="email"
                  class="form-control @error('email') is-invalid @enderror rounded-2" placeholder="البريد الإلكتروني"
              ...
              <p class="fs-sm text-primary-darker mb-0 pt-3">ليس لديك حساب ؟ <a href="{{ route('register') }}"
                  class="fw-medium text-primary-base" data-view="#modal-signup-view">إنشاء حساب</a></p>
            </form>
          </div>
        </div>
      </div>
