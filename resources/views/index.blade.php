            ...
            @foreach ($pharmacies as $pharmacy)
              <article class="card bg-secondary-light rounded-3 card--hover shadow"
                ...
                <a class="btn bg-secondary-dark fw-bold text-dark-50 p-2 pb-3" style="border-radius: 0 0 20px 20px;"
                  href="#" @click.prevent="addOrderModal = true; pharmacy = {{ $pharmacy }}">أطلب دوائك</a>
          ...
          @if (isset($ads))
            @foreach ($ads as $ad)
              <a href="{{ $ad->link }}" class="card bg-secondary-light rounded-3 card--hover shadow"
                ...
                <img src="{{ asset(\App\Enum\AdEnum::AD_PATH . $ad->image) }}" width="80%" class="img-fluid"
                  alt="">
        ...
        <div class="row">
          <div class="col-xl-6 col-lg-7 col-md-8 col-12 m-lg-0 m-md-auto">
            <div class="card p-md-5 card-blur p-3">
              ...
              <p class="fs-lg-5 text-primary-darker mt-2">انضم لصيدلية أونلاين الآن واجعل صيدليتك أونلاين! واحصل على
                مبيعات
                أكثر من
