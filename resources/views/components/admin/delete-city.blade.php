@if (isset($city))
  <x-modal title="حذف المدينة" open="deleteModal">
    <form method="post" action="{{ route('admin.cities.destroy', $city->id) }}" x-ref='deleteForm' class="t-delete-ads">
      @csrf
      @method('DELETE')

      <p class="text-danger">
        هل انت متاكد من حذف المدينة ؟
      </p>
